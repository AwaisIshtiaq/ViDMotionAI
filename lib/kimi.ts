...
export const model = "kimi-k2-thinking";
...
Generate detailed production prompts based on user input.`;
...
Return ONLY valid TypeScript/JSX code wrapped in a code block. Ensure all animations use useCurrentFrame() hook from Remotion.`;
  ...
  const codeMatch = text.match(/```(?:tsx|typescript|jsx|javascript)[\s\S]*?```/);
  if (codeMatch) {
